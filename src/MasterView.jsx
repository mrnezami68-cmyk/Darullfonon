...
function RecordForm({ section, record, onCancel, onSave }) {
  ...
  return <div className="master-form-overlay"><div className="master-form-card"><div className="master-form-head"><div><span className="master-kicker">{record ? 'ویرایش محتوا' : 'ایجاد محتوای جدید'}</span><h2>{record ? `ویرایش ${meta.singular}` : meta.action}</h2></div><button type="button" className="icon-button" onClick={onCancel} aria-label="بستن"><X size={19} /></button></div><form onSubmit={(event) => { event.preventDefault(); if (title.trim()) onSave({ id: record?.id || `${section}-${Date.now()}`, title: title.trim(), detail: detail.trim() || 'بدون توضیح تکمیلی', status, updated: 'همین حالا' }) }}><label>عنوان<input value={title} onChange={(event) => setTitle(event.target.value)} placeholder={`عنوان ${meta.singular}`} autoFocus required /></label><label>خلاصه / جزئیات<textarea value={detail} onChange={(event) => setDetail(event.target.value)} placeholder="یک توضیح کوتاه برای مدیریت محتوا" rows="3" /></label><label>وضعیت انتشار<select value={status} onChange={(event) => setStatus(event.target.value)}><option value="Draft">پیش‌نویس</option><option value="Review">در بررسی</option><option value="Published">منتشر شده</option><option value="Archived">بایگانی شده</option></select></label><div className="master-form-note"><Sparkles size={15} /><span>بعد از ذخیره می‌توانی روابط و جزئیات این محتوا را کامل کنی.</span></div><div className="master-form-actions"><button className="master-secondary-button" type="button" onClick={onCancel}>انصراف</button><button className="master-primary-button" type="submit"><Check size={16} /> ذخیره {meta.singular}</button></div></form></div></div>
}
